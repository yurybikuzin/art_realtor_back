...
impl<'a> Json {
    ...
    pub async fn from_file<P: AsRef<std::path::Path>>(file_path: P) -> Result<Self> {
        ...
        let mut file = File::open(file_path)
            .await
            .map_err(Error::new)
            .context(format!(r#"Json::from_file({file_path:?}): File::open"#))?;
        let mut contents = vec![];
        file.read_to_end(&mut contents)
            .await
            .map_err(Error::new)
            .context(format!(
                r#"Json::from_file({file_path:?}): file.read_to_end"#
            ))?;
        ...
        let value: Value = serde_json::from_str(s)
            .map_err(Error::new)
            .context(format!(
                r#"Json::from_file({file_path:?}): serde_json::from_str {s:?}"#,
            ))?;
        Ok(Self {
            value,
    ...
    // pub fn get_ref<P: AsRef<[By]>>(&self, path_items: P) -> Result<JsonRef> {
    //     let mut ret = JsonRef {
    //         value: &self.value,
    //         path,
    //     };
    ...
    //         ret = {
    //             let get_ret = match path_item {
    //                 By::Key(key) => self.value.get(key),
    //                 By::Index(index) => self.value.get(index),
    //             };
    ...
    pub fn get<P: AsRef<[By]>>(&self, path_items: P) -> Result<Self> {
        ...
        for path_item in path_items.as_ref().iter() {
            ret = {
                let get_ret = match path_item {
                    By::Key(key) => ret.value.get(key),
                    By::Index(index) => ret.value.get(index),
                };
                let value: serde_json::Value = match get_ret {
                    Some(value) => value.clone(),
                    None => {
                        bail!(
                            ...
                            self.path,
                            path_item,
                            serde_json::to_string_pretty(&ret.value)?
                        );
                    }
                };
                let path = self.path.add(path_item.to_owned());
    ...
    //     Ok(Self { value, path })
    // }
    /// same as 'fn get(..)' but inplace
    pub fn go<P: AsRef<[By]>>(&mut self, path_items: P) -> Result<()> {
        for path_item in path_items.as_ref().iter() {
            let value: Option<serde_json::Value> = match path_item {
                By::Key(key) => match &mut self.value {
                    serde_json::Value::Object(ref mut map) => map.remove(key),
                    _ => None,
                },
                By::Index(index) => match &mut self.value {
                    serde_json::Value::Array(ref mut vec) => {
    ...
    pub fn as_str(&self) -> Result<&'_ str> {
        match &self.value {
            Value::String(s) => Ok(s),
            _ => {
                bail!(
    ...
    pub fn as_string(&self, strict: bool) -> Result<String> {
        if strict {
    ...
    pub fn iter_map(&self) -> Result<MapIterator<'_>> {
        let map = self.as_map()?;
    ...
    pub fn iter_vec(&self) -> Result<VecIterator<'_>> {
        let vec = self.as_vec()?;
...
mod tests {
    ...
    async fn test_json_0() -> Result<()> {
        ...
        let eta = format!(
            "{:?}.{:?} expected to be a u16, but: {}",
            file_path, key, 363896
        );
        assert_eq!(tst, eta);
    ...
    async fn test_json_2() -> Result<()> {
        ...
        let err = json.get([By::key(key)]);
        let tst = err.unwrap_err().downcast::<String>().unwrap();
        let eta = format!(
            "{:?}: not found .{:?} at {}",
            file_path,
            key,
            serde_json::to_string_pretty(&json.value)?
        );
    ...
    async fn test_json_7() -> Result<()> {
        ...
        // modifications.items[0].specification.blocks[2].params[2].value
        match json.get([
            By::key("modifications"),
            By::key("items"),
            By::index(0),
            ...
            Ok(value) => {
                ...
                let tst: f64 = match value.parse_as_f64() {
                    Err(err) => error!("{}:{}: {err}", file!(), line!()),
                    Ok(tst) => {
    ...
    async fn test_json_b() -> Result<()> {
        ...
        assert_eq!(
            titles,
            ["Все объявления в России", "Транспорт", "Автомобили"]
        );
    ...
    async fn test_json_c() -> Result<()> {
        ...
        let err = value.parse_as_u8();
        let tst = err.unwrap_err().downcast::<String>().unwrap();
        let eta = r#""json"."some" expected to be a u8 or String parseable to u8, but: "thing""#
            .to_owned();
        assert_eq!(tst, eta);
